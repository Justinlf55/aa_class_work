...
def bad_years
  ...
  # bad_yrs = Movie.group(:yr).where('score < 8').pluck('yr')
...
def vanity_projects
  ...
  Movie.select("movies.id,movies.title,actors.name").joins(:actors).where("castings.ord = 1 AND actors.id = movies.director_id")
end
