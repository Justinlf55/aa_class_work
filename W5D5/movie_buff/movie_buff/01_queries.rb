...
def harrison_ford
  ...
  Movie.select(:id, :title).joins(:actors).where("actors.name = 'Harrison Ford' AND castings.ord != 1").distinct
end
...
def biggest_cast
  ...
  Movie.select(:id, :title).joins(:castings).group(:id, :title).order("COUNT(castings.id) DESC").limit(3)
